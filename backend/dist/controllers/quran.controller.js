...
export class QuranController {
    static getSurahs(_req, res, next) {
        try {
    ...
    static getJuzList(_req, res, next) {
        try {
    ...
    static async addBookmark(req, res, next) {
        try {
            ...
            const bookmark = await QuranService.addBookmark(userId, surahNumber, ayahNumber, surahName, ayahText);
    ...
    static async updateProgress(req, res, next) {
        try {
            ...
            const progress = await QuranService.updateProgress(userId, surahNumber, ayahNumber, surahName, pageNumber);
            res.status(200).json(sendSuccess({ progress }));
        }
        catch (err) {
            next(err);
    ...
    static streamQuranPdf(_req, res, next) {
        try {
