...
async function processItem(item, decoder, outDir) {
  ...
  // Pitch shift factor (0.74 shifts ~220Hz down to 115-125Hz authentic male baritone chest voice)
  const pitchFactor = 0.74;
  ...
  // 1. Male chest resonance filter: Low Shelf +4.5dB at 200Hz
  const chestFilter = new BiquadFilter('lowshelf', inSampleRate, 200, 4.5, 0.9);
  // 2. High-cut de-esser: High Shelf -6.0dB at 4500Hz (removes female sibilance)
  const deEsser = new BiquadFilter('highshelf', inSampleRate, 4500, -6.0, 0.9);
  // 3. Low-pass filter at 5500Hz for warm, deep male resonance
  ...
  // Encode to standard MP3 using lamejs
  const mp3Encoder = new lamejs.Mp3Encoder(1, inSampleRate, 64);
  const mp3Data = [];
  ...
  fs.writeFileSync(outFile, finalMp3);

  const durationSec = (outLength / inSampleRate).toFixed(2);
  console.log(`[PASS] ${item.id} | ${item.arabic.padEnd(20)} | Male Baritone Audio Generated | ${durationSec}s | ${finalMp3.length} bytes`);
}
...
async function run() {
  ...
  console.log('Generating 30 dedicated standalone Male Arabic MP3 audio files...');
  for (const item of names) {
    await processItem(item, decoder, outDir);
    await new Promise(r => setTimeout(r, 100));
  }
  console.log('All 30 standalone male Arabic MP3 audio files generated successfully!');
}
