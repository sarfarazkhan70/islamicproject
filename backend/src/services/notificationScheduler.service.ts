...
import { CalculationMethod } from './prayerEngine/types.js';
...
export class NotificationSchedulerService {
   ...
   */
  static async scheduleDailyJobsForUser(
    ...
    const calcMethod = (prefs.calculationMethod === 'Makkah' ? 'UmmAlQura' : prefs.calculationMethod) as CalculationMethod;
    ...
    if (prefs.notifications.prayerReminders?.enabled) {
      const prayers: Array<'fajr' | 'zuhr' | 'asr' | 'maghrib' | 'isha'> = [
        'fajr',
        'zuhr',
        'asr',
        'maghrib',
   ...
   */
  static async rebuildUserSchedule(userId: string | mongoose.Types.ObjectId): Promise<void> {
    const prefs = await UserPreferences.findOne({ userId });
    if (!prefs) return;
   ...
   */
  static async dispatchDueJobs(): Promise<{
    processed: number;
    delivered: number;
    failed: number;
    skipped: number;
    ...
    for (const job of pendingJobs) {
      ...
      try {
        const result = await WebPushService.sendToUser(
          job.userId,
          {
   ...
   */
  static async sendTestNotification(userId: string | mongoose.Types.ObjectId): Promise<{
    delivered: number;
    failed: number;
  }> {
