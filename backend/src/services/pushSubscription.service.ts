...
import { UserPreferences, INotificationPreferences } from '../models/UserPreferences.js';
...
export class PushSubscriptionService {
  ...
  static async subscribe(
    ...
    const subscription = await PushSubscription.findOneAndUpdate(
      { userId, endpoint: subscriptionData.endpoint },
      {
        userId,
   ...
   */
  static async getStatus(userId: string | mongoose.Types.ObjectId): Promise<{
  ...
  static async updatePreferences(
    ...
    if (updates.enabled !== undefined) prefs.notifications.enabled = updates.enabled;
    if (updates.jumuahTime !== undefined) prefs.notifications.jumuahTime = updates.jumuahTime;
    if (updates.soundEnabled !== undefined) prefs.notifications.soundEnabled = updates.soundEnabled;
