...
describe('Phase 6 — Quran, Surah Al-Mulk & Azkar Endpoints', () => {
  ...
  describe('1. Quran Metadata & Verified Content', () => {
    ...
    it('should retrieve full verified Surah Al-Mulk (Surah 67) with 30 ayahs and reciter audio', async () => {
      ...
      expect(mulk.ayahs[0].arabic).toContain('بِيَدِهِ ٱلْمُلْكُ');
    ...
    it('should verify Surah Al-Baqarah 2:201 matches authentic Tanzil Uthmani text', async () => {
      ...
      expect(ayah201.arabic).toContain('رَبَّنَآ ءَاتِنَا فِى ٱلدُّنْيَا حَسَنَةً');
      expect(ayah201.arabic).toContain('وَقِنَا عَذَابَ ٱلنَّارِ');
    });
