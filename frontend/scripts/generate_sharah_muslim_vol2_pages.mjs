...
async function generateSharahMuslimVol2Pages() {
  const pdfjs = await import('pdfjs-dist/legacy/build/pdf.mjs');
  const pdfPath = path.resolve(__dirname, '../public/pdf/sharah_sahih_muslim_vol2.pdf');
  const wasmDir = path.resolve(__dirname, '../public/wasm') + '/';
  const outDir = path.resolve(__dirname, '../public/sharah-muslim/jild-2');

  if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir, { recursive: true });
  }

  console.log('Loading Sharh Sahih Muslim Jild 2 PDF from:', pdfPath);
  ...
  console.log(
    `✓ Finished generating Sharh Sahih Muslim Jild 2 pages! Total: ${totalPages} pages in ${totalElapsed}s.`
  );
}

generateSharahMuslimVol2Pages().catch((err) => {
  console.error('Error generating Sharh Sahih Muslim Vol 2 pages:', err);
  process.exit(1);
});
