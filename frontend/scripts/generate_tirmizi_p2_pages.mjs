...
async function main() {
  ...
  const pdfPath = path.resolve(__dirname, '../public/pdf/jami_at_tirmizi_vol1_part2.pdf');
  const wasmDir = path.resolve(__dirname, '../public/wasm') + '/';
  const outDir = path.resolve(__dirname, '../public/tirmizi/vol2/pages');
  const coverDir = path.resolve(__dirname, '../public/tirmizi/vol2');
  const cropsDir = path.resolve(__dirname, 'p2_crops');

  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  if (!fs.existsSync(cropsDir)) fs.mkdirSync(cropsDir, { recursive: true });

  console.log('Loading Tirmizi Vol 1 Part 2 PDF from:', pdfPath);
