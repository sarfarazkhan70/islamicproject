...
async function main() {
  ...
  const pdfPath = path.resolve(__dirname, '../public/pdf/jami_at_tirmizi_vol1_part3.pdf');
  const wasmDir = path.resolve(__dirname, '../public/wasm') + '/';
  const outDir = path.resolve(__dirname, '../public/tirmizi/vol3/pages');
  const coverDir = path.resolve(__dirname, '../public/tirmizi/vol3');
  const cropsDir = path.resolve(__dirname, 'p3_crops');

  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  if (!fs.existsSync(coverDir)) fs.mkdirSync(coverDir, { recursive: true });
  if (!fs.existsSync(cropsDir)) fs.mkdirSync(cropsDir, { recursive: true });

  console.log('Loading Tirmizi Vol 1 Part 3 PDF from:', pdfPath);
