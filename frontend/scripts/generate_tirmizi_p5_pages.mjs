...
async function main() {
  ...
  const pdfPath = path.resolve(__dirname, '../public/pdf/jami_at_tirmizi_vol2_part2.pdf');
  const wasmDir = path.resolve(__dirname, '../public/wasm') + '/';
  const outDir = path.resolve(__dirname, '../public/tirmizi/vol5/pages');
  const coverDir = path.resolve(__dirname, '../public/tirmizi/vol5');
  ...
  console.log('Loading Tirmizi Vol 2 Part 2 (Jild 5) PDF from:', pdfPath);
