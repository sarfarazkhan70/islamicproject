...
async function main() {
  ...
  const outDir = path.resolve(__dirname, '../public/tirmizi/vol1/pages');

  if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir, { recursive: true });
  }

  console.log('Loading Tirmizi Vol 1 Part 1 PDF from:', pdfPath);
  ...
  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    ...
    if (pageNum % 25 === 0 || pageNum === totalPages || pageNum <= 10) {
