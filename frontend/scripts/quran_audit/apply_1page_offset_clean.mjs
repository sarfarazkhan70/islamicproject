...
// Initial extra cover page = 1 (PDF Page 1 is Green Cover, Page 2 is Quran Page 1)
const OFFSET = 1;
...
for (let i = 0; i < surahs.length; i++) {
  surahs[i].pageStart = SURAH_PHYSICAL_PAGES[i] - OFFSET;
}

for (let j = 0; j < juzs.length; j++) {
  juzs[j].pageStart = JUZ_PHYSICAL_PAGES[j] - OFFSET;
}

const fileHeader = `/**
 * Quran Static Metadata and Registries
 * Fully aligned with Quran.com API v4 standard
 * Mushaf PDF Mapping for Zia-ul-Quran / Subcontinent 9-Line Mushaf (1124 pages total, 1 initial cover page, 1123 Quran text pages: Page 1 = PDF Page 2)
 */
...
// Complete 114 Surahs Directory (Quran Page 1 = Physical PDF Page 2)
export const SURAHS_LIST: SurahMeta[] = ${JSON.stringify(surahs, null, 2)};

// 30 Paras / Juz Directory (Quran Page 1 = Physical PDF Page 2)
export const JUZ_LIST: JuzMeta[] = ${JSON.stringify(juzs, null, 2)};

export const TOTAL_MUSHAF_PDF_PAGES = 1123;
export const PDF_PHYSICAL_TOTAL_PAGES = 1124;
export const PDF_INITIAL_COVER_OFFSET = 1;
...
console.log('Regenerated src/data/quranData.ts cleanly with 1-page cover offset (Quran Page 1 = PDF Page 2)!');
