...
import {
  ...
  calculateSolarTransitHours,
  calculateHourAngle,
  calculateAsrAltitudeDeg,
...
export function calculatePrayerTimes(params: PrayerCalculationParams): DailyPrayerTimesResult {
  ...
  const madhhab: SunniMadhhab = options.madhhab || 'hanafi';
  const method: CalculationMethod = options.calculationMethod || 'Karachi';
  const highLatRule: HighLatitudeRule = options.highLatitudeRule || 'TwilightAngle';
  ...
  const baseSolarNoonHours = calculateSolarTransitHours(
    longitude,
    tzOffsetHours,
    solarCoords.equationOfTimeMinutes
  );
  ...
  const sunriseHourAngle = calculateHourAngle(
    STANDARD_HORIZON_REFRACTION_DEG,
    latitude,
  ...
  const asrAltitudeDeg = calculateAsrAltitudeDeg(
    shadowMultiplier,
    latitude,
    solarCoords.declinationDeg
  );
  const asrHourAngle = calculateHourAngle(
    asrAltitudeDeg,
    latitude,
    solarCoords.declinationRad
  );
  ...
  const fajrHourAngle = calculateHourAngle(
    -methodParams.fajrAngle,
    latitude,
    solarCoords.declinationRad
  );
  const rawFajrHours = solarNoonHours - fajrHourAngle.hourAngleHours;
  const fajrAdjusted = adjustHighLatitudeTime(
    'fajr',
    rawFajrHours,
    sunriseHours,
    sunsetHours,
  ...
  if (methodParams.ishaIntervalMinutes) {
    ...
    const ishaHourAngle = calculateHourAngle(
      -(methodParams.ishaAngle || 18),
      latitude,
      solarCoords.declinationRad
    );
    rawIshaHours = solarNoonHours + ishaHourAngle.hourAngleHours;
  }
  const ishaAdjusted = adjustHighLatitudeTime(
    'isha',
    rawIshaHours,
    sunriseHours,
    sunsetHours,
  ...
  const derived = calculateDerivedWindows(
    sunriseHours,
    solarNoonHours,
    sunsetHours,
    fajrHours
  ...
  const prayers: CalculatedPrayerSlot[] = [
    {
      key: 'fajr',
      name: 'Fajr',
      arabicName: 'الفجر',
  ...
  let currentPrayer: CalculatedPrayerSlot | undefined;
  let nextPrayer: CalculatedPrayerSlot | undefined;
  ...
  let timeToNextPrayerSeconds: number | undefined;
  let timeToNextPrayerFormatted: string | undefined;
  if (nextPrayer) {
...
export function calculateMonthlyPrayerTimes(params: {
  ...
  longitude: number;
  timezone?: string;
  options?: PrayerCalculationParams['options'];
}): DailyPrayerTimesResult[] {
  const { year, month, latitude, longitude, timezone, options } = params;
  const daysInMonth = new Date(year, month, 0).getDate();
  const results: DailyPrayerTimesResult[] = [];
