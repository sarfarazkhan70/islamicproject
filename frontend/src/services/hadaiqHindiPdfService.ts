// ============================================================================
// HADAIQ-E-BAKHSHISH HINDI PDF SERVICE: High-Resolution Canvas Rendering Engine
// Renders authentic scanned pages of Hadaiq-e-Bakhshish Hindi Edition (All 498 Pages)
// with crisp High-DPI typography, caching & concurrency queue.
// ============================================================================

import * as pdfjsLib from 'pdfjs-dist';
import { LOCAL_HADAIQ_HINDI_PDF_PATH } from '../data/hadaiqHindiData';
...
// Concurrency queue to ensure orderly, high-performance canvas rendering across all 498 pages
...
export class HadaiqHindiPdfService {
  /**
  ...
  public static getDocument(): Promise<pdfjsLib.PDFDocumentProxy> {
    if (!cachedDocPromise) {
      const loadingTask = pdfjsLib.getDocument({
        url: LOCAL_HADAIQ_HINDI_PDF_PATH,
        wasmUrl: '/wasm/',
        cMapUrl: '/cmaps/',
        cMapPacked: true,
      ...
      cachedDocPromise = loadingTask.promise.catch((err) => {
        ...
        pageCache.clear();
        console.error('Failed to load Hadaiq-e-Bakhshish Hindi PDF document:', err);
        throw err;
      });
    }
  ...
  /**
   * Get cached PDFPageProxy for a specific page number (1 to 498)
   */
  ...
  /**
   * Render a specific Hadaiq-e-Bakhshish Hindi page to a canvas element with High-DPI and custom scale.
   * Uses concurrency queue for smooth, non-blocking scrolling across all 498 pages.
   */
