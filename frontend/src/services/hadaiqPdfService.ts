// ============================================================================
// HADAIQ-E-BAKHSHISH PDF SERVICE: High-Resolution Canvas Rendering Engine
// Renders authentic scanned pages of Hadaiq-e-Bakhshish (All 454 Pages)
// with crisp High-DPI typography, caching & concurrency queue.
// ============================================================================

import * as pdfjsLib from 'pdfjs-dist';
import { LOCAL_HADAIQ_PDF_PATH } from '../data/hadaiqData';
...
// Concurrency queue to ensure orderly, high-performance canvas rendering across all 454 pages
...
export class HadaiqPdfService {
  ...
  public static getDocument(): Promise<pdfjsLib.PDFDocumentProxy> {
    if (!cachedDocPromise) {
      ...
      cachedDocPromise = loadingTask.promise.catch((err) => {
        ...
        pageCache.clear();
        console.error('Failed to load Hadaiq-e-Bakhshish PDF document:', err);
        throw err;
      });
    }
  ...
  /**
   * Get cached PDFPageProxy for a specific page number (1 to 454)
   */
  ...
  /**
   * Render a specific Hadaiq-e-Bakhshish page to a canvas element with High-DPI and custom scale.
   * Uses concurrency queue for smooth, non-blocking scrolling across all 454 pages.
   */
