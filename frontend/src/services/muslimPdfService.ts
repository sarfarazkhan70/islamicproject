// ============================================================================
// SAHIH MUSLIM PDF SERVICE: High-Resolution Multi-Page Canvas Rendering Engine
// Renders authentic scanned pages of Sahih Muslim in Urdu (Vol 1 - All 453 Pages)
// with automatic orientation preservation, High-DPI typography & page caching.
// ============================================================================

import * as pdfjsLib from 'pdfjs-dist';
import { getMuslimVolume } from '../data/muslimData';
...
export class MuslimPdfService {
  ...
  public static getDocument(volumeNumber: number = 1): Promise<pdfjsLib.PDFDocumentProxy> {
    ...
    if (!cachedDocPromises.has(vol)) {
      ...
      const promise = loadingTask.promise.catch((err) => {
        ...
        console.error(`Failed to load Sahih Muslim Vol ${vol} PDF document:`, err);
        throw err;
      });
  ...
  public static getPageImageUrl(pageNumber: number, volumeNumber: number = 1): string {
    ...
    if (volMeta.volumeNumber === 1) {
      return `/muslim/pages/page_${validPage}.webp`;
    }
    return `/muslim/vol${volMeta.volumeNumber}/pages/page_${validPage}.webp`;
  }
  ...
  /**
   * Render a specific Sahih Muslim page to a canvas element with High-DPI, correct orientation,
   * custom scale, and automatic inpainting cleanup of external watermark URLs.
