 ...
 * bidirectional conversion, Maghrib-based day boundary, and sacred Islamic events.
 * ==============================================================================
 */
...
} from '../core/prayerEngine/astronomical.js';
...
/**
 * Reads user's active/cached location from central storage
 */
export function getClientStoredLocation(): HijriLocationParam {
  try {
    if (typeof localStorage !== 'undefined') {
      const raw = localStorage.getItem('islamic_prayer_central_location_v1');
      if (raw) {
  ...
  return { latitude: 28.6139, longitude: 77.209, maghribOffsetMinutes: 0 }; // Default baseline
}
...
/**
 * Returns the centralized active Hijri date.
 * If user selected Manual mode, returns the exact manual date.
 * If Automatic mode, calculates the Maghrib-based live date + user adjustment.
 */
export function getCentralHijriDate(): HijriDate {
  try {
    const settings = useSettingsStore.getState();
    const loc = useLocationStore.getState();
    const tz = loc.timezone || getResolvedTimezone();

    if (settings.hijriMode === 'manual' && settings.manualHijriDate) {
      const { day, month, year } = settings.manualHijriDate;
      const validMonth = Math.min(12, Math.max(1, month || 1));
      const validDay = Math.min(30, Math.max(1, day || 1));
      const validYear = year || 1448;
      const monthObj = HIJRI_MONTHS[validMonth - 1] || HIJRI_MONTHS[0];
      const matchedEvent = ISLAMIC_EVENTS.find(
        (e) => e.hijriMonth === validMonth && e.hijriDay === validDay
      );
      const now = new Date();
      const localComp = getLocalDateComponents(now, tz);
      const dateStr = `${localComp.year}-${String(localComp.month).padStart(2, '0')}-${String(localComp.day).padStart(2, '0')}`;
      ...
      return {
        ...
        day: validDay,
        formatted: `${validDay} ${monthObj.name} ${validYear} AH`,
        formattedArabic: `${validDay} ${monthObj.arabicName} ${validYear} هـ`,
    ...
    const adjustment = settings.hijriAdjustment || 0;
    return getCurrentHijriDate(adjustment, loc.timezone, {
  ...
  if (typeof gregorianDate === 'string') {
    if (/^\d{4}-\d{2}-\d{2}$/.test(gregorianDate)) {
      ...
      const loc =
        ...
        (typeof window !== 'undefined'
          ? getClientStoredLocation()
          : { latitude: 28.6139, longitude: 77.209, maghribOffsetMinutes: 0 });
